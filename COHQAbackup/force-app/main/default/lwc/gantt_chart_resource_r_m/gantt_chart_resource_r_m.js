...
import getProjects from "@salesforce/apex/ganttChartRM.getProjects";
import verifyAllocationandSave from "@salesforce/apex/ganttChartRM.overallocated";
import saveAllocation from "@salesforce/apex/ganttChartRM.saveAllocation";
import deleteAllocation from "@salesforce/apex/ganttChartRM.deleteAllocation";

export default class GanttChartResourceRM extends LightningElement {
  ...
  // percentofHours from allocated Team Assignments
  @api totalPercentofHours;
    ...
    if (this.projectId) {
      ...
      getProjects()
        .then(projects => {
          ...
          /* Commenting out Unavailable Value
          projects.unshift({
            value: "Unavailable",
            label: "Unavailable"
          }); */
  ...
  addAllocationModalSuccess() {
    ...
    this._saveAllocation({
      ...
      numberofHours: this.addAllocationData.numberofHours,
      percent: this.addAllocationData.Time_of_Time_Requested__c
    })
      .then(() => {
  ...
  _verifyAllocationandSave(allocation) {
    if (
      null == allocation.projectId &&
      null != this.projectId &&
      !allocation.status
    ...
    return verifyAllocationandSave(allocation)
      .then(result => {
        this.totalPercentofHours = result;
        console.log('totalPercentofHours2 = ', this.totalPercentofHours);
        if(this.totalPercentofHours > 100){
          this.dispatchEvent(
            new ShowToastEvent({
              message: 'Resource is overallocated',
              variant: "error"
            })
          );
        }
  ...
  handleModalEditClick(event) {
    this.editAllocationData = {
      ...
      numberofHours: this.menuData.allocation.Number_of_Hours__c,
      percent: this.menuData.allocation.Time_of_Time_Requested__c,
  ...
  editAllocationModalSuccess() {
    ...
    this._verifyAllocationandSave({
      ...
      numberofHours: this.editAllocationData.numberofHours,
      percent: this.editAllocationData.Time_of_Time_Requested__c
    })
      .then(result => {
        this.totalPercentofHours = result;
        console.log('totalPercentofHours: ', this.totalPercentofHours);
        this.editAllocationData = {};
