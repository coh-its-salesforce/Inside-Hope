/*
* This trigger processes HD_Evaluation_Request object.
* 
* Balaji Rao
* City of Hope
* Copyright (c) 2019
...
trigger COH_HD_Evaluation_Request on HD_Evaluation_Request__c (before insert,before update) {//,after insert,after update
		  public set<String> patientNames=new set<String>();
      for (HD_Evaluation_Request__c hdevalRequestObj : Trigger.new) {
         ...
         if (Trigger.isUpdate && hdevalRequestObj.NP_Triage_Date_Time__c != trigger.oldMap.get(hdevalRequestObj.Id).NP_Triage_Date_Time__c)
         {
             ...
             else if(typeofRequestObj == 'Enhanced (test) Interpretation Review'){
               if((hdevalRequestObj.NP_Triage_Date_Time__c).hour() < 16)
                {
                    hdevalRequestObj.COH_Review_Due_Date__c = COH_EMP_Strategy_Util.AddBusinessDays(hdevalRequestObj.NP_Triage_Date_Time__c.date(), 2);
                }
        ...
        if(hdevalRequestObj.NP_Triage_Date_Time__c!=null && hdevalRequestObj.Report_Sent_to_CM_Date_Time__c!=null){
            
            hdevalRequestObj.Days_B_W_NP_Triage_Report_Sent__c=Integer.valueof(COHUtil.CalculateWorkingDays(hdevalRequestObj.NP_Triage_Date_Time__c.date(), hdevalRequestObj.Report_Sent_to_CM_Date_Time__c.date()));
            /*COHUtil daysDiff=new COHUtil();
            hdevalRequestObj.Days_B_W_NP_Triage_Report_Sent__c=Integer.valueof(daysDiff.CalculateWorkingDays(hdevalRequestObj.NP_Triage_Date_Time__c.date(), hdevalRequestObj.Report_Sent_to_CM_Date_Time__c.date()));*/
            ...
            Double iReportSenthour = hdevalRequestObj.Report_Sent_to_CM_Date_Time__c.hour();
            Double iTimeDiff;
            DateTime NPTriageDate=hdevalRequestObj.NP_Triage_Date_Time__c;//hdevalRequestObj
            String dayOfWeek1=NPTriageDate.format('EEEE');
            DateTime ReportSentDate=hdevalRequestObj.Report_Sent_to_CM_Date_Time__c;
            ...
            else if(dayOfWeek2=='Saturday' || dayOfWeek2=='Sunday' || holidays.contains(hdevalRequestObj.Report_Sent_to_CM_Date_Time__c.date())){
                iTimeDiff= 24 - iNPTriagehour ;
            }
            else if((dayOfWeek1=='Saturday' || dayOfWeek1=='Sunday' || holidays.contains(hdevalRequestObj.NP_Triage_Date_Time__c.date())) && (dayOfWeek2=='Saturday' || dayOfWeek2=='Sunday' || holidays.contains(hdevalRequestObj.Report_Sent_to_CM_Date_Time__c.date()))){
          ...
          //Report_Sent_to_CM_Date_Time__c
        if(hdevalRequestObj.Submitted_for_Review_Date_Time__c!=null && hdevalRequestObj.Report_Sent_to_CM_Date_Time__c!=null){
            
           //Days_Between_Submitted_Report_Sent__c
            hdevalRequestObj.Days_Between_Submitted_Report_Sent__c=Integer.valueof(COHUtil.CalculateWorkingDays(hdevalRequestObj.Submitted_for_Review_Date_Time__c.date(), hdevalRequestObj.Report_Sent_to_CM_Date_Time__c.date()));
            ...
            else if((dayOfWeek1=='Saturday' || dayOfWeek1=='Sunday' || holidays.contains(hdevalRequestObj.Submitted_for_Review_Date_Time__c.date())) && (dayOfWeek2=='Saturday' || dayOfWeek2=='Sunday' || holidays.contains(hdevalRequestObj.Report_Sent_to_CM_Date_Time__c.date()))){
    ...
    /*if(trigger.isBefore && trigger.isInsert){
        ...
        for (HD_Evaluation_Request__c hdevalRequestObj1 : [select id,name,Account_Name__c from HD_Evaluation_Request__c where Account_Name__c!=null AND  LastModifiedDate=THIS_YEAR ]) {
            
            patientNames.add(hdevalRequestObj1.Account_Name__c);
        }
        
        for (HD_Evaluation_Request__c hdevalRequestObj2 : trigger.new){
            
            if(patientNames.contains(hdevalRequestObj2.Account_Name__c)){
                
                hdevalRequestObj2.Exclude_From_Report__c=true;
