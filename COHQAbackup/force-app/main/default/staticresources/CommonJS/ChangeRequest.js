...
var hideServiceEle = true;
var hideCloseDate = true;      
var labelIncidentPlural = '';	
...
var NewBtnHandler = function(){ 
	openPage("ChangeRequestPage",ChangeRequestHeader,ChangeRequestHeader); activeAllWindowFlag=false;
...
var SaveBtnHandler = function(){
    if(!inProgress){
		...
		var reviewCommentsComponentData = document.getElementById(reviewCommentsComponent).value.trim();
		...
		if(reviewCommentsComponentData.length > 255 ){
			Ext.MessageBox.show({ msg: labelReviewComments + ': ' + AddNote.Labels.TextAreaOverflow, buttons: Ext.MessageBox.OK});         
			return;
		}		
...
function hideServiceRequestEle(){
	var srEle = document.getElementById('serviceReqEle');
	if(srEle != null){
		if(hideServiceEle){
			srEle.style.display = 'none';
		}else{
			srEle.style.display = '';
		}
	}
 }
 function hideCloseDateEle(){
	var closeDateEle = document.getElementById('closeDateEle');
	if(closeDateEle != null){
		if(hideCloseDate){
		closeDateEle.style.display = 'none';
		}else{
			closeDateEle.style.display = '';
		}
	}
 }
