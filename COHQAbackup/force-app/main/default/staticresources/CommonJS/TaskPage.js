...
var NewBtnHandler = function(button,event) {  
	...
	openPage('TaskPage?isCreateable='+isCreateable+'&isDeletable='+isDeletable+'&isUpdateable='+isUpdateable,labelTaskWindowHeaderSearchPage,labelTaskWindowHeaderSearchPage);activeAllWindowFlag=false; 
};
var isValid ='True';
...
var SaveBtnHandler = function(button,event) { 
	...
	if(!inProgress){    
		...
		var taskDescription = getDescriptionName(); 
        var taskDescriptionValue=document.getElementById(taskDescription).value;
	
		var taskResolution = getResolutionName(); 
		var taskResolutionValue=document.getElementById(taskResolution).value;
		...
		var countDescription = taskDescriptionValue.length;
		if(!Ext.isIE && taskDescriptionValue != null){
			if(null != taskDescriptionValue.match(/[^\n]*\n[^\n]*/gi)){
				countDescription = countDescription + taskDescriptionValue.match(/[^\n]*\n[^\n]*/gi).length;
			}
		}
		var countResolution = taskResolutionValue.length;
		if(!Ext.isIE && taskResolutionValue != null){
			if(null != taskResolutionValue.match(/[^\n]*\n[^\n]*/gi)){
				countResolution = countResolution + taskResolutionValue.match(/[^\n]*\n[^\n]*/gi).length;
			}
		}
		if(countDescription <= 32000 && countResolution <= 32000){
			if(isPriorityChanged == 'true' && taskId != '' && taskId != null && impactName != '' && urgencyName != ''){
			 ...
			 if(countDescription > 32000){
				Ext.MessageBox.show({msg: labelDescription + ': ' + labelTextOverflow, width: 300,buttons:Ext.MessageBox.OK});
			}else if(countResolution > 32000){
				Ext.MessageBox.show({msg: labelResolution + ': ' + labelTextOverflow, width: 300,buttons:Ext.MessageBox.OK});
			}
...
var taskDuedatetimeId = getTaskDuedatetimeId();												 
var ResetBtnHandler = function(button,event) { document.getElementById(taskDuedatetimeId).value = ''; resetTask(); };
...
/////////////// // End IE7-8 Related Changes
   ...
   if(taskId == null || taskId == ''){
	...
	//added for template checkbox value
	if(taskId != null && taskId != ''){
...
function afterSaveCloseAction(){ 
	...
	if(errormsg==labelSavedSuccessfully){            
		openPopupWithTitle('CloseTask?taskId='+getTaskId()+'&isLookup=true' + '&categoryId='+ categoryIdForClose ,closeTaskNext,labelCloseTask,380,600);
	}else {
		showError();
	}
...
function callCopyPage(){
	...
	window.parent.addTab("TaskPage?copyId=" + taskId + '&isCreateable='+isCreateable+'&isDeletable='+isDeletable+'&isUpdateable='+isUpdateable,labelTaskWindowHeaderSearchPage,labelTaskWindowHeaderSearchPage);
}
