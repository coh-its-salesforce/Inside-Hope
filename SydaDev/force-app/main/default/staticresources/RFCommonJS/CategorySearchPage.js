...
{	
   ...
   function setCategoryId(catId,catName,cUrgency){  
	...
	var arrColsEmpty = new Array(3);	
	arrColsEmpty[0] = {id:'StandardDescription', header: labelStandardDescriptionAndResolution, dataIndex: 'StandardDescription'};
	arrColsEmpty[1] = {id:'DescriptionID', header:'DescriptionID', hidden: true,dataIndex: 'DescriptionID'};
	arrColsEmpty[2] = {id:'ResolutionID', header:'ResolutionID', hidden: true,dataIndex: 'ResolutionID'};
	
desGridEmpty = new Ext.grid.GridPanel({
    store: ds1,
    border:true,
    cm: new Ext.grid.ColumnModel({
            defaults: {sortable: true},
            columns:arrColsEmpty
        }),
	viewConfig: {scrollOffset: 0,forceFit: true,emptyText: labelMsgNoRecordFoundSearchPage,deferEmptyText: false},
...
resGrid = new Ext.grid.GridPanel({
   ...
   sm: new Ext.grid.RowSelectionModel({singleSelect:true}),   
    ...
    var displayEmptyPanel = new Ext.Panel({
        height       : 175,
        layout       : 'vbox',
	...
	if(getIsStandardDescription() == 'true') {
		if(descriptionData != ''){
			var displayPanel = new Ext.Panel({
				...
				disabled: isShowFooterDisabled(),
				defaults     : { flex : 1 }, //auto stretch
				items        : displayEmptyPanel
			});
			document.getElementById("OKButton").style.visibility = 'hidden'; 
			document.getElementById("cancelButton").value = OkButton;
		}
	}
	}
...
{		
	var desCheck = document.getElementById("desCheckBox").checked;
	var resCheck = document.getElementById("resCheckBox").checked;
	
	if(desCheck && resCheck){	
		if(descriptionString != '' && resolutionString != ''){
			saveResolutionDescription(descriptionString,resolutionString);		
		}
		else{
			if(descriptionString == '' && resolutionString == ''){
				Ext.MessageBox.show({ msg: labelValidateDescriptionResolution, title: labelWarningTitleSearchPage, buttons: Ext.MessageBox.OK,width:300,icon: Ext.MessageBox.WARNING});
			}
			else if(descriptionString == ''){
				Ext.MessageBox.show({ msg: labelValidateDescription, title:labelWarningTitleSearchPage, buttons: Ext.MessageBox.OK,width:300,icon: Ext.MessageBox.WARNING});
			}
			else if(resolutionString == ''){
				Ext.MessageBox.show({ msg: labelValidateResolution, title: labelWarningTitleSearchPage, buttons: Ext.MessageBox.OK,width:300,icon: Ext.MessageBox.WARNING});
			}
		}
	}
	if(desCheck && !resCheck){		
...
function refresh(){
	window.opener.location.href = '/'+objId;
    window.close();
}
