	...
	if(foundTab == 0) {
		...
		if(initialPageRef != null) {			
		   ...
		   if(pageTarget == "KnowledgeSearch"){
				if(pageRef.indexOf('useListViewTarget=true') != -1){
					pageRef = pageRef.substring(0,pageRef.indexOf('?')+1)+'useListViewTarget=false&'+pageRef.substring(pageRef.indexOf('useListViewTarget=true')+23);					
				}else{
   ...
   if(n.leaf){
		...
		while(tabsList.length>i) {
			...
			if(tab == n.text) {
				...
				if(pageTarget.indexOf('KnowledgeSearch') != -1 || pageTarget.indexOf('SearchPage') != -1){
					if(pageTarget.indexOf('?') != -1){
		...
		if(foundTab==0) {
		 if((linkArray[n.attributes.pageRef]) == null ){
			 ...
			 if(pageRef!=null && pageRef.indexOf('KnowledgeSearch') != -1){
						
						pageRef = pageRef.substring(0,pageRef.indexOf('KnowledgeSearch')) + 'KnowledgeSearch?standardLayout=true';
						
			 }
			 if(pageRef!=null&&pageRef.indexOf('NavigatorPage')!=-1){
			 
				if(pageRef.indexOf('target=ChangeSchedule') != -1){
				
					pageRef = pageRef + '&standardLayout=true';
			 	}
			 }
			 if(pageRef!=null && pageRef.indexOf('SearchPage') != -1){
				
				if(pageRef.indexOf('popupId=Client') != -1){
					pageRef = pageRef + '%26standardLayout=true';
				}
				if(pageRef.indexOf('popupId=Template') != -1){
					pageRef = pageRef + '%26standardLayout=true';
				}
                                 if(pageRef.indexOf('popupId=Organization') != -1){
					pageRef = pageRef + '&standardLayout=true';
				}
			 }
	   ...
	   //openedTabTitle = '';fix for defect #63808
	}else{
		if(n.expanded){
			n.collapse();
		}else{
...
Ext.onReady(function() {
	...
	// This is the default tab that we should show on launching Remedyforce User mgmt tab.
	addNewTab(labelGlobalSearch, labelGlobalSearch, 'NavigatorPage?title=' + labelGlobalSearch + '&amp;target=KnowledgeSearch?standardLayout=true');
    }
});            
...
function callNode() {
	var url;
	var title;
	var page=getUrlParameter('node');
	var node = Ext.getCmp('ConfigurationTree').getNodeById(nodeId);
	if(getUrlParameter('BMC_BaseElementid')!=null && getUrlParameter('BMC_BaseElementid')!='' ){
		url=node.attributes.pageRef+'&moduleName='+getUrlParameter('moduleName')+'&BMC_BaseElementid='+getUrlParameter('BMC_BaseElementid')+'&fromstdCI=true';
	}
	else {
		url=node.attributes.pageRef+'&moduleName='+getUrlParameter('moduleName')+'&id='+getUrlParameter('id')+'&fromstdCI=true';
	}
	url=url.replace(url.substring(url.indexOf('target=',0)+7,url.indexOf('?',url.indexOf('target=',0))),page);
	title=url.substring(url.indexOf('title=',0)+6,url.indexOf('&',url.indexOf('title=',0)));
	addNewTab(page,title,url);
}
