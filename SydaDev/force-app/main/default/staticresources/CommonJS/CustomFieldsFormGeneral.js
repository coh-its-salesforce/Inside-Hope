...
function ValidateForm()
{
	...
	for(var i=0;i<__CMDBControlMap.length;i++)
	{
		...
		if(ctrl.Validate && ctrl.Required && ctrl.Element.value.trim()=='')
		{
			...
			Ext.Msg.alert('', CustomFieldrequiredFieldMsg);
			submit=false;
			if(!submit){			
				return submit;
			}
		...
		if(ctrl.Validate && !ctrl.Readonly && ctrl.DisplayType=="EMAIL"){
			if(ctrl.Element.value.trim()!=''){
				var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
				var emailid = ctrl.Element.value.trim();
				if(!reg.test(emailid)){
					var ctrlLabel = __CMDBControlMap.item(__FIELD_LABEL_PREFIX+ctrl.UniqueID).Element.innerHTML;
					Ext.Msg.alert('', CustomFieldInvalidEmail+' '+ctrlLabel);
					submit=false;
				}
		...
		if(ctrl.Validate && !ctrl.Readonly && (ctrl.DisplayType=="CURRENCY" || ctrl.DisplayType=="PERCENT" || ctrl.DisplayType=="PHONE" || ctrl.DisplayType=="DOUBLE" || ctrl.DisplayType=="NUMBER")){
			if(ctrl.Element.value.trim()!=''){
				var pattern = /^-?\d+(\.\d+)?$/;
				var currency = ctrl.Element.value.trim();
				if(!currency.match(pattern)){
					var ctrlLabel = __CMDBControlMap.item(__FIELD_LABEL_PREFIX+ctrl.UniqueID).Element.innerHTML;
					if(ctrl.DisplayType == "DOUBLE"){
						ctrl.DisplayType = "NUMBER";
					}
					Ext.Msg.alert('', CustomFieldCorrectValueErrorMsg +' '+ctrlLabel);
					submit=false;
				}
				...
				if(ctrl.DisplayType=="PHONE"){
					...
					if (!(ctrl.Element.value.match(RegExPatternNumeric))) {
						submit=false;
						Ext.Msg.alert('', CustomFieldCorrectValueErrorMsg+' '+__CMDBControlMap.item(__FIELD_LABEL_PREFIX+ctrl.UniqueID).Element.innerHTML);
					}			
				}
		...
		if(ctrl.Validate && !ctrl.Readonly && ctrl.DisplayType=="DATE"){
			...
			/*	var reg = /^\d{1,2}\/\d{1,2}\/\d{4}$/;
				...
				if(!reg.test(cDate)){
					var ctrlLabel = __CMDBControlMap.item(__FIELD_LABEL_PREFIX+ctrl.UniqueID).Element.innerHTML;
					Ext.Msg.alert('', CustomFieldCMDBDateError+' '+ctrlLabel);
					submit=false;
				}
...
function showExpansionWindow(ctrl){
	...
	var param = { value: old_data,readOnly:ctrl.ForControl.Readonly, cls:'InputVirtualReadOnly' };
