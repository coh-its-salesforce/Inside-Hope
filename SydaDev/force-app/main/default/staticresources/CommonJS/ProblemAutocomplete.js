function findIt(sel,divId,field,evt)
    {      
          ...
          // list = null;
		   ...
		   var id=''+ field.id;
		   var customLeft= Ext.isIE8 ? 817:815;
  ...
  function SelectValue(currentSelect, divId){
        ...
        if (cIndex > -1) {
           ...
           if(divId.indexOf('Impact') != -1){
				...
				document.getElementById(ProblemPageComp.ComponentVars.ImpactID).value=option.id;
				FetchPriorityHTTP();
            }else if(divId.indexOf('Urgency') != -1){
                 document.getElementById(urgId).value=option.value;
				document.getElementById(ProblemPageComp.ComponentVars.UrgencyID).value=option.id;
				FetchPriorityHTTP();
            }else if(divId.indexOf('Category') != -1){
                 document.getElementById(catIdVal).value =option.value;
				var len = list.length;
                for(var i = 0; i< len; i++){ 
                    var dataObj = list[i];
                    if(dataObj['id'] == option.id){
                        document.getElementById(ProblemPageComp.ComponentVars.CategoryID).value=option.id;
					    document.getElementById(ProblemPageComp.ComponentVars.CategoryFKUrgency).value=dataObj['u'];
						FetchUrgencyHTTP();
                 ...
                 for(var i = 0; i< len; i++){
                    ...
                    if(dataObj['i'] == option.id){
					   ...
					   document.getElementById(ProblemPageComp.ComponentVars.UserID).value = option.id;
					   FetchUrgencyHTTP();
                       break;  
                    }
                 }
