...
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
        ...
        var msConfig = [{
            legend: 'Available Profiles',
            draggable: true,
            droppable: true,
            width: 100,
            height: 100
        },{
            legend: 'Selected Profiles',
            droppable: true,
            draggable: true,
            width: 100,
            height: 100
        ...
        var lbl = new Ext.form.Label({text:this.firstText});
        ...
        var el=icons.getEl();
        this.addIcon = el.createChild({tag:'img', src:this.iconRight,id:'iconRightId', style:{cursor:'pointer', margin:'4px'}});
        el.createChild({tag: 'br'});
        this.removeIcon = el.createChild({tag:'img', src:this.iconLeft,id:'iconLeftId', style:{cursor:'pointer', margin:'4px'}});

        var el1=icons1.getEl();
        this.toTopIcon = el1.createChild({tag:'img', src:this.iconTop,id:'iconTopId', style:{cursor:'pointer', margin:'4px'}});
        el1.createChild({tag: 'br'});
        this.upIcon = el1.createChild({tag:'img', src:this.iconUp,id:'iconUpId', style:{cursor:'pointer', margin:'4px'}});
        el1.createChild({tag: 'br'});
        this.downIcon = el1.createChild({tag:'img', src:this.iconDown, id:'iconDownId',style:{cursor:'pointer', margin:'4px'}});
        el1.createChild({tag: 'br'});
        this.toBottomIcon = el1.createChild({tag:'img', src:this.iconBottom,id:'iconBottomId', style:{cursor:'pointer', margin:'4px'}});
            ...
            while(selectionsArray.length>i){
             record = this.fromMultiselect.view.store.getAt(selectionsArray[i]);
                ...
                if(queryParam.toLowerCase()== 'top requests'){ 
                    addToRightDataStore(record); //added by Samruddhi for Top Requests
                }
            i++;
            }
            if(!this.allowDup)selectionsArray = [];
            var j=0;
            while(records.length>j){
                record = records[j];
                if(this.allowDup){
                    ...
                    if(queryParam.toLowerCase()== 'top requests'){ //added by Samruddhi for adding prefix to right store
                        var textVal;
                        this.fromMultiselect.view.store.remove(record);
                        this.toMultiselect.view.store.add(record);
                        selectionsArray.push((this.toMultiselect.view.store.getCount() - 1));
                        var cnt=this.toMultiselect.view.store.getCount() - 1;
                        if(record.get('type')==templateLabel){
                            textVal = prefix + record.get('text');
                        }else{
                            textVal = record.get('text');
                        }
                        this.toMultiselect.view.store.getAt(cnt).set('text',Ext.util.Format.ellipsis(textVal,40+prefix.length));
                    }
                    else{
                        this.fromMultiselect.view.store.remove(record);
                        this.toMultiselect.view.store.add(record);
    ...
    toFrom : function() {
        ...
        if (selectionsArray.length > 0) {
            ...
            if(queryParam.toLowerCase()== 'top requests'){    //to identify if all selected items are of same type
                var recType=this.toMultiselect.view.store.getAt(selectionsArray[0]).get('type');
                 for(i=0; i<records.length;i++) {
                     if(recType==records[i].get('type'))
                         similarItems++;    
                }
            }
            selectionsArray = [];
            ...
            while(records.length>j) {
                record = records[j];
                if(queryParam.toLowerCase()== 'top requests'){ //added by Samruddhi for Top Requests
                    if(record.get('text').indexOf(prefix) != -1 && record.get('text').indexOf(prefix) == 0){ // remove prefix
                        var prefixIndex = record.get('text').indexOf(prefix) + prefix.length;
                        record.set('text',record.get('text').substring(prefixIndex,record.get('text').length));
                    }
            ...
            //Added to dynamically change the left store based on the type of record moved
            if(queryParam.toLowerCase()== 'top requests'){
                if(records.length==1 || similarItems==records.length){
                    refreshDataStore(records[0].get('type'));
                    Ext.getCmp('typeCombo').setValue(records[0].get('type'));
                    type=records[0].get('type');
                }
                else
                    refreshDataStore(type);
            }    
        }
        this.fromMultiselect.view.refresh();
        this.toMultiselect.view.refresh();
