    ...
    {      
           ...
           if(id.indexOf('Impact') != -1){
             ...
             if(field.value!='' && field.value!=null){
			  fetchCategoryData(sel,divId,field,iKeyCode);
			 }else{
...
function findAutoCompleteSelectOptions(sel,divId,field,iKeyCode)
 {  
		   ...
		   if(field.value.length > 0){
               ...
               if(selobj.options.length >0){
                    ...
                    if(Ext.isIE7){
                        ...
                        selobj.style.height = (cnt*13)+4+'px';
					}
           ...
           if(id.indexOf('firstname') != -1){
            ...
            if(list !='[]' && list!=''){
               ...
               if(useCurrentList){
                    ...
                    document.getElementById(divId).style.visibility='hidden';
                    fetchClientData(clientFor,field.value, 'userVal'); 
               }
            }else{
                document.getElementById(divId).style.visibility='hidden'; 
                fetchClientData(clientFor,field.value, 'userVal');
    ...
    function fetchClientData(clientFor, searchString, userVal){
		var con = new Ext.data.Connection();
        con.request({
            url: 'ClientDataPage',
            method: 'POST',  
            params: {"clientFor":clientFor,"startString":searchString, "userVal":userVal},
  ...
  function SelectValue(currentSelect, divId){
        ...
        if (cIndex > -1) {
           ...
           if(divId.indexOf('Impact') != -1){
				...
				document.getElementById(ChangePageComp.ComponentVars.ImpactID).value=option.id;
				FetchPriorityHTTP();
            }else if(divId.indexOf('Urgency') != -1){
                 document.getElementById(urgId).value=option.value;
				document.getElementById(ChangePageComp.ComponentVars.UrgencyID).value=option.id;
				FetchPriorityHTTP();
            }else if(divId.indexOf('Category') != -1){
                ...
                for(var i = 0; i< len; i++){ 
                    ...
                    if(dataObj['id'] == option.id){
                        document.getElementById(ChangePageComp.ComponentVars.CategoryID).value=option.id;
					    document.getElementById(ChangePageComp.ComponentVars.CategoryFKUrgency).value=dataObj['u'];
						FetchUrgencyHTTP();
                 ...
                 for(var i = 0; i< len; i++){
                    ...
                    if(dataObj['i'] == option.id){
					   ...
					   document.getElementById(ChangePageComp.ComponentVars.UserID).value = option.id;
					    if(dataObj['urg'] != 'null'){
					   		document.getElementById( ChangePageComp.ComponentVars.clientFKUrgency).value=dataObj['urg'];
					   }else{
					   		document.getElementById( ChangePageComp.ComponentVars.clientFKUrgency).value='';
					   }
					  FetchUrgencyHTTP();
                       break;  
                    }
